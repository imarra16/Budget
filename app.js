...
function payerRec(id){
  ...
  r.dernierMois=moisCourant(); r.statut='paye';
  r.opId=uid();
  D.ops.unshift({id:r.opId,type:'out',montant:r.montant,libelle:r.libelle,compteId:r.compteId,date:auj()});
  maj();
}
/* sauter : marque l'echeance comme passee sans creer de depense */
function sauterRec(id){
  const r=D.recs.find(x=>x.id===id); if(!r)return;
  r.dernierMois=moisCourant(); r.statut='saute'; r.opId=null;
  maj();
}
/* annule le paiement ou le saut du mois en cours */
function annulerRec(id){
  const r=D.recs.find(x=>x.id===id); if(!r)return;
  if(r.statut==='paye'&&r.opId) D.ops=D.ops.filter(o=>o.id!==r.opId);
  r.dernierMois=null; r.statut=null; r.opId=null;
  maj();
}
